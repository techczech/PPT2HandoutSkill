...
cd my-handout
npm install
npm run init   # Creates empty data files from templates
```

Then proceed with Step 1.

**Note:** `npm run build` also runs `init` automatically, so you can skip the explicit `init` step if you're going straight to a build. The init script copies `.template` files in `src/data/` to their runtime names (e.g., `sessionInfo.ts.template` → `sessionInfo.ts`) only if the target doesn't already exist.
...
**Data files** (in `src/data/`, created by `init-data.js` from `.template` files, then overwritten by skill steps):
- `presentation.json` — slide content (overwritten by processMedia.js from sourcematerials)
- `entities.json` — AI-extracted entities (generated in Step 5)
- `sessionInfo.ts` — presentation metadata (generated in Step 6)
- `lectureNotes.ts` — lecture notes (generated by LectureNotesSkill, Step 10)

**Media assets** (in `public/assets/`, copied by processMedia.js from sourcematerials):
- `images/slides/` — slide images
- `images/icons/` — SmartArt icons
- `screenshots/` — slide screenshots (from generate-screenshots.py)
- `videos/` — presentation videos

**Type definitions** (`src/data/*.types.ts`) ARE tracked — they contain only TypeScript interfaces, no data.

**Never commit data files or media assets to the template repo.** Each presentation generates its own via the workflow.
